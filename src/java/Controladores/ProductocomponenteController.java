...
import Repositorios.ProductocomponenteFacade;
...
public class ProductocomponenteController implements Serializable {
    ...
    @EJB
    private Repositorios.ProductocomponenteFacade ejbFacade;
    ...
    public Productocomponente getSelected() {
        if (current == null) {
            ...
            current.setProductocomponentePK(new Entidades.ProductocomponentePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getProductocomponentePK().setCodProducto(current.getProductopropio().getCodProducto());
            current.getProductocomponentePK().setCodComponente(current.getComponente().getCodComponente());
    ...
    public Productocomponente getProductocomponente(Entidades.ProductocomponentePK id) {
    ...
    public static class ProductocomponenteControllerConverter implements Converter {
        ...
        Entidades.ProductocomponentePK getKey(String value) {
            Entidades.ProductocomponentePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new Entidades.ProductocomponentePK();
            key.setCodProducto(Integer.parseInt(values[0]));
            key.setCodComponente(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(Entidades.ProductocomponentePK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getCodProducto());
            sb.append(SEPARATOR);
