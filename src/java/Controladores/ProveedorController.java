...
import Repositorios.ProveedorFacade;
...
public class ProveedorController implements Serializable {
    ...
    @EJB
    private Repositorios.ProveedorFacade ejbFacade;
    ...
    public static SelectItem[] getSelectItems(List<Proveedor> entities, boolean selectOne) {
        int size = selectOne ? entities.size() + 1 : entities.size();
        SelectItem[] items = new SelectItem[size];
        int i = 0;
        if (selectOne) {
            items[0] = new SelectItem("-1", "Selecciona un proveedor");
            i++;
        }
        for (Proveedor x : entities) {
            items[i++] = new SelectItem(x, x.getNomProveedor()/*+ " " + x.getApellido2() + ", " + x.getNomAutor()*/);
        }
        return items;
    }
}
